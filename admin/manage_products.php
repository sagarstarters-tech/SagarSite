...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $min_order_qty = max(1, intval($_POST['min_order_qty'] ?? 1));
        $bulk_price_raw = $_POST['bulk_price'] ?? '';
        $bulk_price = ($bulk_price_raw !== '' && $bulk_price_raw !== null) ? floatval($bulk_price_raw) : null;
        $bulk_price_sql = ($bulk_price !== null && $bulk_price >= 0) ? $bulk_price : 'NULL';
        ...
        if (isset($_FILES['image'])) {
            if ($_FILES['image']['error'] === 0) {
                ...
                if (!move_uploaded_file($_FILES['image']['tmp_name'], $upload_target . $image)) {
                    $error = "Failed to move uploaded main image. Please check directory permissions.";
                    $image = null;
                } else {
                    $image = 'uploads/media/images/' . $image; // store full relative path
                }
                ...
                for ($i = 0; $i < $count; $i++) {
                    if ($_FILES['gallery']['error'][$i] === 0) {
                        ...
                        if (move_uploaded_file($_FILES['gallery']['tmp_name'][$i], $upload_target . $g_fname)) {
                            ...
                            $conn->query("INSERT INTO product_images (product_id, image, position) VALUES ($product_id, '$g_esc', $i)");
                        }
                    }
                }
            }
        ...
        if (isset($_FILES['image'])) {
            if ($_FILES['image']['error'] === 0) {
                $ext = strtolower(pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION));
                $new_img_fname = 'prod_' . uniqid() . '.' . $ext;
                // Save to uploads/media/images/ — consistent with media library
                $upload_target = '../uploads/media/images/';
                if (!is_dir($upload_target)) mkdir($upload_target, 0755, true);
                if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_target . $new_img_fname)) {
                    $new_image_path = 'uploads/media/images/' . $new_img_fname;
                    ...
                    if ($img_q && $img_q['image']) {
                        ...
                        if ($other_prod_refs === 0 && $gal_refs === 0) {
                            $try1 = '../' . ltrim($img_q['image'], '/');
                            $try2 = '../uploads/images/' . basename($img_q['image']);
                            $try3 = '../assets/images/' . basename($img_q['image']);
                    ...
                    $_SESSION['flash_error'] = "Failed to move uploaded main image. Please check directory permissions.";
                }
            } elseif ($_FILES['image']['error'] !== 4) {
                ...
                <tbody>
                    <?php if($products && $products->num_rows > 0): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-dark">
                                <div class="fw-bold fs-6"><?php echo $global_currency; ?><?php echo number_format($p['price'], 2); ?></div>
                                <?php if (!empty($p['bulk_price']) && (float)$p['bulk_price'] > 0): ?>
                                    <small class="text-muted d-block" style="font-size: 0.75rem;"><span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-1 py-0"><i class="fas fa-layer-group me-1"></i>Bulk: <?php echo $global_currency . number_format($p['bulk_price'], 2); ?><?php echo !empty($p['bulk_min_qty']) ? ' ('.$p['bulk_min_qty'].'+)' : ''; ?></span></small>
                                <?php endif; ?>
    ...
    <?php echo form_nonce_input('edit_product'); ?>
        <div class="modal-body p-4">
            ...
            <div class="row">
                ...
                <!-- B2B & Bulk Order Pricing Group -->
                <div class="col-12 mb-3">
                    <div class="bg-light p-3 rounded border">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <h6 class="fw-bold mb-0 text-dark"><i class="fas fa-boxes me-2 text-primary"></i>Minimum Order Quantity (MOQ) & Bulk Order Pricing</h6>
                            <span class="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 px-2 py-1 small">B2B / Wholesale</span>
                        </div>
