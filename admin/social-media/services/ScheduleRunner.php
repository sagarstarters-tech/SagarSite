...
class ScheduleRunner {
    ...
    public function executeSchedule($scheduleOrId, bool $allProducts = true): int {
        ...
        foreach ($products as $product) {
            ...
            $imgUrl = '';
            // Build image URL directly from products.image (main image only — no gallery fallback)
            // resolve_product_image_url does disk checks and falls back to product_images (gallery)
            // which is undesirable for social media posts
            if (!empty($rawImg)) {
                if (strpos($rawImg, 'http://') === 0 || strpos($rawImg, 'https://') === 0) {
                    ...
                    // Bare filename — check common directories
